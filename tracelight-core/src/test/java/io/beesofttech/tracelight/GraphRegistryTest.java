...
class GraphRegistryTest {
    ...
    @Test
    void recordEntryHitIncrementsEntryCounter() {
        GraphRegistry registry = new GraphRegistry();
        registry.ensureEntry("GET /order", "GET /order");

        assertThat(registry.recordEntryHit("GET /order")).isEqualTo(1);
        assertThat(registry.recordEntryHit("GET /order")).isEqualTo(2);
        assertThat(registry.node("GET /order").count()).isEqualTo(2);
    }

    @Test
    void recordEntryHitIgnoresUnknownNode() {
        GraphRegistry registry = new GraphRegistry();

        assertThat(registry.recordEntryHit("nope")).isEqualTo(0);
        assertThat(registry.node("nope")).isNull();
    }

    @Test
    void resetClearsEntryHitCounter() {
        GraphRegistry registry = new GraphRegistry();
        registry.ensureEntry("GET /order", "GET /order");
        registry.recordEntryHit("GET /order");

        registry.resetCounters();

        assertThat(registry.node("GET /order").count()).isEqualTo(0);
    }
