...
/**
 * Triangle for an arrowhead whose tip sits at `tip`, pointing along `dir` (need not be a unit
 * vector). `base` is pulled back from the tip by `size` along the flow — draw the edge stroke to
 * it so the line terminates under the arrowhead instead of poking through the tip. `left`/`right`
 * are the trailing wing corners; the filled poly is [tip, left, right].
 */
export function arrowhead(
  ...
  return {
    base,
    left: { x: base.x + px * halfW, y: base.y + py * halfW },
    right: { x: base.x - px * halfW, y: base.y - py * halfW },
  };
}
