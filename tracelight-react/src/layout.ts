...
import { NODE_MIN_DIST } from './gl/constants';
...
export async function layoutGraph(
  ...
  const graph = {
    ...
    layoutOptions: {
      ...
      // Same-layer gap derived from the radial minimum-distance constraint: vertical
      // neighbors sit nodeHeight + gap apart center-to-center, so fresh layouts already
      // satisfy NODE_MIN_DIST and the scene's load-time relaxation is a near-no-op.
      'elk.spacing.nodeNode': String(NODE_MIN_DIST - options.nodeHeight),
